  ...
  public class TestADirectory : TestHelper {
    ...
    public async Task Delete() {
      using(var act = new Act()) {
        ...
        Assert.IsFalse(aDirectory.ExistsSync());
        await aDirectory.Create();
        Assert.IsTrue(await aDirectory.Exists());
        Assert.IsTrue(aDirectory.ExistsSync());
    ...
    public async Task ExistedPathAlias() {
      using(var act = new Act()) {
        ...
        await aDir.Create();
        AFile aFile = aDir.CFile(new string[] { "existedAlias.txt", "existedAliasalias.txt" });
        await aFile.WriteAllText("Hi");
        Assert.IsTrue(await aDir.Exists());
        Assert.IsTrue(await aFile.Exists());
        await aDir.Delete();
        Assert.IsFalse(await aDir.Exists());
        Assert.IsFalse(await aFile.Exists());
        await ADirectory.FromFullPath(path).CDirectory("existedaliasalias").CFile("existedAliasalias.txt").WriteAllText("Hello");
        Assert.IsTrue(await aDir.Exists());
        Assert.IsTrue(await aFile.Exists());
        Console.WriteLine(await aFile.Path());
        Assert.IsTrue((await aFile.Path()).EndsWith("existedaliasalias\\existedAliasalias.txt"));
        Assert.AreEqual(1, (await aDir.GetFiles()).Count);
