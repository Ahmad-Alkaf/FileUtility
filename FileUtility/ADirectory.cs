...
namespace FileUtility {
    ...
    public class ADirectory : AFileDirectory {
            ...
            if(await PathIfExists() is string path) {
                if(recycleBin) {
                    ...
                    try {
                        ...
                        // Fallback: Directory.Delete can fail with IOException (access denied) when
                        // the folder is managed by OneDrive or another sync agent that holds file handles.
                        // cmd.exe rmdir /s /q uses OS-level deletion which handles this better.
                        if(!recycleBin)
                            await DeleteViaCmd(path);
                        else
                            throw e;
                    } catch(Exception e) {
                        if(!recycleBin)
        ...
        /// <summary>
        /// Fallback deletion using cmd.exe rmdir /s /q. Handles directories locked by
        /// OneDrive or other sync agents that prevent Directory.Delete from succeeding.
        /// </summary>
        private static async Task DeleteViaCmd(string path) {
            await Task.Run(() => {
                ...
                using(var proc = System.Diagnostics.Process.Start(psi)) {
                    proc.WaitForExit(300_000); // 300 second timeout
                    if(!proc.HasExited)
                        proc.Kill();
                    if(Directory.Exists(path))
                        throw new IOException("Failed to delete directory via cmd.exe fallback: " + path);
                }
            });
        }
